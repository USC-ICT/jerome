...
var transform = exports.transform = function(scxmlDoc) {
    ...
    return {
		...
		name: pm.platform.dom.getAttribute(root,"name"),
        datamodel: datamodel
    };
};
...
function getScope(transition){
    //Transition scope is normally the least common ancestor (lca).
    //Here we deal with the special case where a transition originates at 
    //an orthogonal component (child of a parallel state) 
    //and targets itself, or one of its descendants.
    var source = idToStateMap[transition.source];

    var transitionOriginatesFromOrthogonalComponentAndTargetsSelfOrDescendant = 
            source.parent &&    //root state won't have parent
                idToStateMap[source.parent].kind === stateKinds.PARALLEL && //is it a child of a parallel state
                transition.targets && //do its transitions target itself or its descendants
                    transition.targets.map(function(targetId){return idToStateMap[targetId];}).every(
                    function(target){
                        return (source === target) || 
                            (source.descendants.map(function(id){return idToStateMap[id];}).indexOf(target) > -1);});

    if(!transition.targets){
        return transition.source; 
    }else if(transitionOriginatesFromOrthogonalComponentAndTargetsSelfOrDescendant){
        return transition.source; 
    }else{
        return transition.lca;
    }
}
